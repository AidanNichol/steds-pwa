...
export function NewBookingCell(props) {
  // const [state, dispatch] = useTestUpdater();
  const { walkId, memberId, full, fee, i } = props;
  const bookingChange = useStoreActions((actions) =>
    actions.accountStatus.bookingChange
  );
  const updateStatus = useStoreActions((actions) =>
    actions.walkBookingStatus.updateStatus
  );
  ...
  return (
    <div
      className={"bookingcell book member" + i}
      key={"Q" + walkId + memberId}
    >
