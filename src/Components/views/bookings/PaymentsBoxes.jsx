...
const PaymentsBoxesUnstyled = (props) => {
  const PushUpSelect = ({ options, value, changed, className }) => {
    ...
    return (
      <PushUp onClick={toggleShow} className={'payment-types ' + className + open}>
  ...
  const { accountId, credit, owing } = props;
  const paymentReceived = useStoreActions((a) => a.accountStatus.paymentReceived);
