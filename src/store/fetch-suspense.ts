...
const getDefaultFetchFunction = (): FetchFunction => {
  if (typeof window === 'undefined') {
    return (): Promise<Response> => {
      return Promise.reject(
        new Error(
          'Cannot find `window`. Use `createUseFetch` to provide a custom `fetch` function.',
        ),
      );
    };
  }
  if (typeof window.fetch === 'undefined') {
    return (): Promise<Response> => {
      return Promise.reject(
        new Error(
          'Cannot find `window.fetch`. Use `createUseFetch` to provide a custom `fetch` function.',
        ),
      );
    };
  }
...
export const createUseFetch: CreateUseFetch = (
  ...
  function myUseFetch(
    ...
    if (typeof options === 'number') {
      return myUseFetch(input, init, { lifespan: options });
    }

    const { metadata = false, lifespan = 0 } = options;

    // Check each cache by this myUseFetch hook.
    ...
    const cache: FetchCache = {
      ...
      fetch: fetch(input, init)
        // Parse the response.
        .then(
          (response: Response): Promise<FetchResponse> => {
